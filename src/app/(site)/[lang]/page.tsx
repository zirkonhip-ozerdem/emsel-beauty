...
const localizedHomeShowcase: Record<Locale, HomeShowcaseContent> = {
  ...
  en: {
    ...
    productEyebrow: "Emsel Selection",
  },
  de: {
...
export default async function HomePage({ params }: HomePageProps) {
  ...
  const [homepageServices, homepageProducts] = await Promise.all([
    getHomepageServices(),
    getHomepageProducts(),
  ]);
  ...
  const products = homepageProducts.map((product) => {
    ...
    return {
      ...
      description:
        stripProductHtmlTags(localized.shortDescription) ||
        stripProductHtmlTags(localized.description),
      image: product.imageUrl || "/background/back-1.jpeg",
      href: `/${locale}/products/${localized.slug}`,
    };
  });
          ...
          <div className="home-product-grid">
            {products.map((product) => (
              <article key={product.href} className="home-product-card">
                ...
                <Link href={product.href} className="home-product-button">
